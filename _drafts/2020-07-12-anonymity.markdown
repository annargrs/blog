...
Note that the "quality" is defined as "with more potential for growth". I would argue that that itself is a process largely influenced not only by conference publication, but also how the community receives the work afterwards. That is in itself also influenced by both factors that create the bias towards big labs: post-publication PR (they have more resources to write blog posts, give talks) and unconscious biases (prompting e.g. the campaign for <a href="https://twitter.com/intent/tweet?button_hashtag=CiteBlackWomen&ref_src=twsrc%5Etfw" class="twitter-hashtag-button" data-show-count="false">#CiteBlackWomen</a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>). I cannot even start doing that topic justice here, but as an example - here is recent evidence specifically for NLP that "female first authors are cited less than male first authors, even when controlling for experience and area of research" {% cite Mohammad_2020_Gender_Gap_in_Natural_Language_Processing_Research_Disparities_in_Authorship_and_Citations %}. 

All of that means that even if a paper from an unknown lab and/or underrepresented community gets published at ACL, it will likely not make such a big splash as a Stanford one and/or by overrepresented community -- and that means that fewer people will adopt and build upon its results. Hence the "potential for growth" is never going to be the same, even if the papers are of similar quality.

Here's what we do know for a fact about the effect of double-blind review:
...
* The bias against underrepresented communities should be reduced; e.g. female first authors get a better chance {% cite RobertsVerhoef_2016_Double-blind_reviewing_at_EvoLang_11_reveals_gender_bias %}.
* Geographic diversity in NLP should support language inclusivity {% cite JoshiSantyEtAl_2020_State_and_Fate_of_Linguistic_Diversity_and_Inclusion_in_NLP_World %}.

Granted, not everyone has these goals high up in the list of priorities, but, judging by the overall direction of ACL 2020, and the presence of diversity&inclusion chairs in the recent conferences, the ACL organization now does. And as the social movement towards diversity&inclusion grows, hopefully the community will also gradually become more receptive towards papers from non-famous labs, actively building on them. I personally would bet that having a variety of backgrounds and perspectives would accelerate rather than slow down NLP progress.
